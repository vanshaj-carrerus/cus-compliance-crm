...
export function CrmProvider({
  ...
  const persist = useCallback(async (snap: Snapshot, silent = false) => {
    ...
    try {
      const { res, data } = await apiFetch<{ error?: string }>("/api/crm/save", {
        method: "PUT",
        json: snap,
      });
    ...
    (async () => {
      try {
        const [cRes, hRes, sRes] = await Promise.all([
          apiFetch<{ candidates?: Candidate[]; error?: string }>(
            "/api/candidates"
          ),
          apiFetch<{ history?: PaymentHistory[]; error?: string }>(
            "/api/history"
          ),
          apiFetch<{ settings?: CrmSettings; error?: string }>("/api/settings"),
        ]);
        if (!cRes.res.ok || !hRes.res.ok || !sRes.res.ok) {
          throw new Error(
            cRes.data.error ||
              hRes.data.error ||
              sRes.data.error ||
              "Failed to load CRM data from API"
          );
        }
        if (cancelled) return;
        const c = (cRes.data.candidates || []).map((x: Candidate) =>
          normalizeCandidate(x)
        );
        setCandidatesState(c);
        setHistoryState(hRes.data.history || []);
        setSettingsState(mergeSettings(sRes.data.settings));
        setReady(true);
        setSaveState("saved");
        ...
        if (!cancelled) {
          setError(
            e instanceof Error
              ? e.message
              : "Failed to load CRM. Check API connection."
          );
          setReady(true);
        }
      }
